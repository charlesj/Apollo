  ...
  saveCompletion() {
    ...
    apolloServer.invoke("AddCompletedChecklist", payload).then(() => {
      Notifier.show({
        ...
        message: "Successfully save checklist.",
      });
      this.setState({
        active: false
      });
  ...
  render() {
    ...
    return (<div className="checklistCompletionContainer">
        ...
        return <option value={ct.id} key={ct.id}>{ct.name}</option>
          ...
          <button className="pt-button pt-large" onClick={this.toggleActive}>Cancel</button>
          <button className="pt-button pt-large pt-intent-success" onClick={this.saveCompletion}>Save</button>
        </div>
