...
function NoteListingDisplay(props) {
  return (
    <div className="notesListings">
      <div
        className={getNoteListingClasses(props.selectedId, -1)}
        onClick={props.onChangeNote.bind(null, -1)}
      >
        New Note
      </div>
      {props.notes.map(n => {
        return (
          <div
            className={getNoteListingClasses(props.selectedId, n.id)}
            key={n.id}
            onClick={props.onChangeNote.bind(null, n.id)}
          >
            {n.name}
          </div>
        );
...
class Notebooks extends Component {
  ...
  loadNotes() {
    apolloServer.invoke("getNotes", {}).then(notes => {
      this.setState({
        notes
      });
    });
  ...
  save() {
    if (this.state.selectedNoteId === -1) {
      apolloServer
        ...
        .then(() => {
          NotifySuccess("Successfully saved a new note.");
          this.setState({
            selectedNoteId: -1,
            noteName: "",
            noteBody: ""
      ...
      apolloServer
        ...
        .then(() => {
          NotifySuccess("Successfully updated note.");
        });
    }
  }
      ...
      <div className="notebooksContainer">
        ...
        <div className="notebookDisplay">
          <div className="notebookCommandBar">
            <button className="pt-button pt-intent-primary" onClick={this.save}>
              Save
            </button>
          </div>
          <div>
